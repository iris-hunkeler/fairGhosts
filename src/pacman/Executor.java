...
public class Executor {
    ...
    public static void main(String[] args) {
        ...
        // define MsPacMan controller
        Controller<MOVE> msPacManController = new HumanController(new pacman.controllers.KeyBoardInput());
        // Controller<MOVE> msPacManController = new NearestPillPacMan();
        // Controller<MOVE> msPacManController = new StarterPacMan();

        // define ghost controller
        Controller<EnumMap<GHOST, MOVE>> ghostController = new FairGhosts();

        // run game in different modes

//        runMultipleGamesInBatchModeForTesting(exec, msPacManController, ghostController);

//        runGameInSynchronousMode(exec, msPacManController, ghostController);

        runGameInAsynchronousMode(exec, msPacManController, ghostController);

//        runGameInCompetitionMode(exec, msPacManController, ghostController);

//        recordGame(exec, msPacManController, ghostController);
    }

    private static void runMultipleGamesInBatchModeForTesting(Executor exec, Controller<MOVE> msPacManController, Controller<EnumMap<GHOST, MOVE>> ghostController) {
        int numTrials = 10;
        exec.runExperiment(msPacManController, ghostController, numTrials);
    }

    /**
     * game waits until controllers respond.
     */
    private static void runGameInSynchronousMode(Executor exec, Controller<MOVE> msPacManController, Controller<EnumMap<GHOST, MOVE>> ghostController) {
        int delay = 100;
        boolean visual = true;
        exec.runGame(msPacManController, ghostController, visual, delay);
    }


    private static void runGameInAsynchronousMode(Executor exec, Controller<MOVE> pacManController, Controller<EnumMap<GHOST, MOVE>> ghostController) {
        boolean visual = true;
     ...
     */
    private static void runGameInCompetitionMode(Executor exec, Controller<MOVE> msPacManController, Controller<EnumMap<GHOST, MOVE>> ghostController) {
        boolean visual = true;
        boolean fixedTime = false;
        exec.runGameTimedSpeedOptimised(msPacManController, ghostController, fixedTime, visual);
    }

    private static void recordGame(Executor exec, Controller<MOVE> msPacManController, Controller<EnumMap<GHOST, MOVE>> ghostController) {
        //run game in asynchronous mode and record it to file for replay at a later stage.
        boolean visual = true;
        String fileName = "replay.txt";
        exec.runGameTimedRecorded(msPacManController, ghostController, visual, fileName);
